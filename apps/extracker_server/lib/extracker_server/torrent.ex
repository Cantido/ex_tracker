alias ExtrackerServer.Peer

defmodule ExtrackerServer.Torrent do
  @moduledoc """
  ...
  ## Examples
      ...
      iex> torrent = ExtrackerServer.Torrent.new()
      iex> Enum.to_list(torrent.peers)
      []
  """
  def new do
    %ExtrackerServer.Torrent{}
  end

  @doc """
  """
  def new(peers) when is_list(peers) do
    %ExtrackerServer.Torrent{peers: Map.new(peers, fn(x) -> {x.peer_id, x} end)}
  end
